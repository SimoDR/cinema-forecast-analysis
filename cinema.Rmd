...
it_cinema = it_cinema[which(it_cinema$Date < "2020-03-01"),]
ggplot( data = it_cinema, aes( Date, Overall.Gross )) + geom_line() 
...
```{r fig.width=7}
ggplot( data = it_cinema[which(it_cinema$Year == 2010 | it_cinema$Year == 2011),], aes( Date, Overall.Gross )) + 
  geom_line() + 
...
```{r}
ggplot( data = it_cinema[which(it_cinema$Date >= "2010-12-12" & it_cinema$Date <= "2011-01-15"),], aes( Date, Overall.Gross )) + 
  geom_line() + 
...
```{r}
christmas <- it_cinema[which(it_cinema$Week >= 48 | it_cinema$Week <= 5),]
christmas <- christmas %>%
    group_by(Year) %>%
    summarize(Overall.Gross = sum(Overall.Gross))

autumn <- it_cinema[which(it_cinema$Week >= 39 & it_cinema$Week <= 48),]
autumn <- autumn %>%
    group_by(Year) %>%
    summarize(Overall.Gross = sum(Overall.Gross))

summer <- it_cinema[which(it_cinema$Week >= 23 & it_cinema$Week <= 38),]
summer <- summer %>%
    group_by(Year) %>%
    summarize(Overall.Gross = sum(Overall.Gross))
...
```{r}
bm_GC<-BM(it_cinema$Overall.Gross,display = T,
          prelimestimates = c(sum(it_cinema$Overall.Gross) + 100, 0.001, 0.1),)
summary(bm_GC)
...
```{r}
dates <- c(it_cinema$Date,seq(as.Date("2020-03-01"), as.Date("2023-03-01"), "week"))
...
ggplot( ) +
  geom_line(data = it_cinema, aes( Date, Overall.Gross), color="red") +
...
```{r}
it_cinema = it_cinema[which(it_cinema$Date > "2020-03-11"),]
```
...
```{r}
bm_GC<-BM(it_cinema$Overall.Gross,display = T,
          prelimestimates = c(sum(it_cinema$Overall.Gross) + 100, 0.001, 0.01),)
summary(bm_GC)

dates <- it_cinema$Date
pred_bmGC<- predict(bm_GC, newx=c(1:length(dates)))
pred.instGC<- make.instantaneous(pred_bmGC)
pred_bm <- data.frame(cbind(dates,pred.instGC))
pred_bm$dates <- dates
...
```{r}
bm_GC<-GBM(it_cinema$Overall.Gross,display = T,
          shock="rett", nshock = 2,

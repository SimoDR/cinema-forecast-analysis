...
```{r}
arima<- Arima(it_cinema_postcovid$Overall.Gross,order=c(1,1,1))
ggplot( ) +
  geom_line(data = it_cinema_postcovid, aes( Date, Overall.Gross, color="Values")) +
  geom_point(data = it_cinema_postcovid, aes( Date, Overall.Gross, color="Values"),size=1.2)+
...
```
```{r}
fit4 <- gbm(cinema_data$Montly.gross ~ cinema_data$perc + cinema_data$netflix_access,train.fraction = 0.9,n.trees=500)
summary(fit4)
...
ggplot( ) +
  geom_line(data = cinema_data, aes( month, Montly.gross)) +
  geom_line(data = cinema_data, aes(month, Yhat), color="red")

```
```{r}

armax1<- Arima(cinema_data$Montly.gross, xreg=cbind(cinema_data$perc,cinema_data$netflix_access),seasonal=list(order=c(0,1,0), period =12))
res1<- residuals(armax1)
Acf(res1)

ggplot( ) +
  geom_line(data = cinema_data, aes( month, Montly.gross)) +
  geom_line(data = cinema_data, aes(month, fitted(armax1)), color="red")
...
```{r}
cinema.es <- ses(cinema_data$Montly.gross, alpha = .2, h = 100)
plot(cinema.es)
```
Forcasting: prendere serie precovid e fare forcasting, veder come sarebbe andata
  prendere postcovid e fare forcasting lì

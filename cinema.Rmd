...
```{r}
a2<- auto.arima(it_cinema_precovid$Overall.Gross,  seasonal = c(3,3,3))
windows()
ggplot( ) +
  geom_line(data = it_cinema_precovid, aes( Date, Overall.Gross)) +
  geom_line(data = it_cinema_precovid, aes(Date, fitted(a2)), color="red")
a2
windows()
res2<- residuals(a2)
ggtsdisplay(res2)
...
```
```{r}
fit4 <- gbm(cinema_data$Montly.gross ~ cinema_data$perc + cinema_data$netflix_access,bag.fraction = 0.9,train.fraction = 0.9,n.trees=500)
summary(fit4)
...
armax1<- Arima(cinema_data$Montly.gross, xreg=cbind(cinema_data$perc,cinema_data$netflix_access),order=c(0,1,0))
